...
def update_stickfigure(points, fig):
...
def get_dataframes(video_selected):
    if not video_selected:
        # Return all the rows on initial load/no country selected.
        return dash.no_update
    data = df.query('country in @countries_selected')
...
def update_table(playing, currentTime):
    if not playing and currentTime and currentTime > 0:
        df1 = video_frames[int(np.round(currentTime / .04))]
...
@app.callback(Output('graph-im1', 'figure'),
              [Input('video-player', 'playing')],
              [State('video-player', 'currentTime')])
def update_figure(playing, currentTime):
    if not playing and currentTime and currentTime > 0:

        # points = get_coordinates(keypoints[int(np.round(1/.04))])
        df = video_frames[int(np.round(currentTime/.04))]
        fig = go.Figure()
        img_width = 400
        img_height = 400
        scale_factor = 0.5
